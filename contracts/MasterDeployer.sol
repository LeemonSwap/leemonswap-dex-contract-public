...
contract MasterDeployer is Ownable {
    ...
    function deployPool(address _factory, bytes calldata _deployData) external payable returns (address pool) {
        if (!whitelistedFactories[_factory]) revert NotWhitelisted();
        pool = IPoolFactory(_factory).deployPool{ value: msg.value }(_deployData);
        pools[pool] = true;
