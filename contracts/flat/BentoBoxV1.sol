...
import "./boring/BoringRebase.sol";
import "../interfaces/IFlashLoan.sol";
import "../interfaces/IWHBAR.sol";

contract BentoBoxV1 is MasterContractManager, BoringBatchable, SafeHederaTokenService {
    using BoringMath for uint256;
    using BoringMath for uint128;
    ...
    event LogStrategyQueued(address indexed token, IStrategy indexed strategy);
    event LogStrategySet(address indexed token, IStrategy indexed strategy);
    ...
    address private immutable wethToken;
    ...
    mapping(address => Rebase) public totals;

    mapping(address => IStrategy) public strategy;
    mapping(address => IStrategy) public pendingStrategy;
    ...
    function deposit(address token_, address from, address to, uint256 amount, uint256 share) public payable allowed(from)
    returns (uint256 amountOut, uint256 shareOut) {
        ...
        // Effects
        address token = token_ == USE_ETHEREUM ? wethToken : token_;
        Rebase memory total = totals[token];

        // If a new token gets added, the tokenSupply call checks that this is a deployed contract. Needed for security.
        uint256 supply = IERC20(token).totalSupply();
        require(total.elastic != 0 || supply > 0, "BentoBox: No tokens");
        if (share == 0) {
        ...
        total.base = uint128(total.base.add(share.to128()));
        total.elastic = uint128(total.elastic.add(amount.to128()));
        totals[token] = total;
        ...
        if (token_ == USE_ETHEREUM) {
            ...
            IWHBAR(address(wethToken)).deposit{value : amount}(from, address(this));
        } else if (from != address(this)) {
            // X2 - If there is an error, could it cause a DoS. Like balanceOf causing revert. (SWC-113)
            // X2: If the token implementation is faulty or malicious, it may block adding tokens. Good.

            //            token.safeTransferFrom(from, address(this), amount);
            //            safeTransferTokenRouter(address(token), from, address(this), amount);

            safeTransferTokenRouter(token, from, address(this), amount);
        }
    ...
    function withdraw(address token_, address from, address to, uint256 amount, uint256 share) public allowed(from)
    returns (uint256 amountOut, uint256 shareOut) {
        // Checks
        require(to != address(0), "BentoBox: to not set");
        // To avoid a bad UI from burning funds
        ...
        total.elastic = uint128(total.elastic.sub(amount.to128()));
        total.base = uint128(total.base.sub(share.to128()));
    ...
    function transferMultiple(address token, address from, address[] calldata tos, uint256[] calldata shares) public allowed(from) {
        // Checks
    ...
    function flashLoan(IFlashBorrower borrower, address receiver, IERC20 token, uint256 amount, bytes calldata data) public {
        ...
        safeTransferTokenRouter(address(token), msg.sender, receiver, amount);
        borrower.onFlashLoan(msg.sender, token, amount, fee, data);

        require(_tokenBalanceOf(address(token)) >= totals[address(token)].addElastic(fee.to128()), "BentoBox: Wrong amount");
        emit LogFlashLoan(address(borrower), address(token), amount, fee, receiver);
    }
    ...
    // F6 - Check for front-running possibilities, such as the approve function (SWC-114)
    // F6: Slight grieving possible by withdrawing an amount before someone tries to flashloan close to the full amount.
    function batchFlashLoan(IBatchFlashBorrower borrower, address[] calldata receivers, IERC20[] calldata tokens,
    ...
    public {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            safeTransferTokenRouter(address(tokens[i]), msg.sender, receivers[i], amounts[i]);
        }
    ...
    function setStrategyTargetPercentage(address token, uint64 targetPercentage_) public onlyOwner {
        // Checks
    ...
    function setStrategy(address token, IStrategy newStrategy) public onlyOwner {
    ...
    function harvest(address token, bool balance, uint256 maxChangeAmount) public {
        StrategyData memory data = strategyData[token];
        ...
        if (balanceChange > 0) {
            ...
            totals[token].elastic = uint128(totalElastic.to128());
            data.balance = uint128(data.balance.sub(sub.to128()));
            emit LogStrategyLoss(token, sub);
        }
        ...
        if (balance) {
            ...
            if (data.balance < targetBalance) {
                ...
                safeTransferTokenRouter(token, msg.sender, address(_strategy), amountOut);
                data.balance = uint128(data.balance.add(amountOut.to128()));
                _strategy.skim(amountOut);
                emit LogStrategyInvest(token, amountOut);
            } else if (data.balance > targetBalance) {
                uint256 amountIn = uint128(data.balance.sub(targetBalance.to128()));
                ...
                data.balance = uint128(data.balance.sub(actualAmountIn.to128()));
    ...
    function associateWith(address tokenAddress) public onlyOwner {
        safeAssociateToken(address(this), tokenAddress);
    }

    function associateWith(address[] memory tokenAddresses ) public onlyOwner  {
        safeAssociateTokens(address (this),tokenAddresses);
    }
}