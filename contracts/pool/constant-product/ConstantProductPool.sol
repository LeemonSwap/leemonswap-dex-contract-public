...
contract ConstantProductPool is IPool, ReentrancyGuard, SafeHederaTokenService {
    using Bits for uint;
    ...
    constructor() payable {
        ...
        ) = IConstantProductPoolFactory(msg.sender).getDeployData();
    ...
    function _createFungible() internal {
        ...
        myToken.name = string(
            abi.encodePacked("lsCPLP-", token0name, "-", token1name)
        );
        myToken.symbol = string(
            abi.encodePacked("lsCPLP-", token0symbol, "-", token1symbol)
        );
        ...
        returns (IPool.TokenAmount[] memory withdrawnAmounts)
    {
        ...
        safeBurnToken(lpToken, liquidity, new int64[](0));
    ...
    function getAmountIn(
        bytes calldata data
    ) public view override returns (uint256 finalAmountIn) {
