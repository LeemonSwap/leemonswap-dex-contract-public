...
contract ConstantProductPoolFactoryHelper {
    ...
    function getPoolsForTokens(address payable constantProductPoolFactory, address[] calldata tokens)
        external
        view
        ...
        for (uint8 i = 0; i < tokenNumber; i++) {
            for (uint8 j = i + 1; j < tokenNumber; j++) {
                ...
                for (uint256 k = 0; k < pools.length; k++) {
                    ...
                    poolInfo.tokenA = i;
                    poolInfo.tokenB = j;
                    ConstantProductPool pool = ConstantProductPool(payable(pools[k]));
