...
import {HederaIERC20} from "../../interfaces/HederaIERC20.sol";
...
contract StablePool is IPool, ReentrancyGuard, SafeHederaTokenService {
    ...
    constructor() payable {
        (
            ...
            IMasterDeployerV2 _masterDeployer
        ) = IStablePoolFactory(msg.sender).getDeployData();
    ...
    function balanceOf(address account) external view returns (uint256) {
        return ERC20(lpToken).balanceOf(account);
    }

    function symbol() external view returns (string memory) {
        return ERC20(lpToken).symbol();
    }

    function name() external view returns (string memory) {
        return ERC20(lpToken).name();
    }

    function decimals() external view returns (uint8) {
        return ERC20(lpToken).decimals();
    }

    function totalSupply() external view returns (uint256) {
        return ERC20(lpToken).totalSupply();
    }

    function initialize() external {
        if (initialized) return;
        initialized = true;
        safeAssociateToken(address(this), token0);
        safeAssociateToken(address(this), token1);
    }

    function _createFungible() internal {
        uint supplyKeyType;
        ...
        string memory token0name = HederaIERC20(token0).name();
        string memory token1name = HederaIERC20(token1).name();

        string memory token0symbol = HederaIERC20(token0).symbol();
        string memory token1symbol = HederaIERC20(token1).symbol();

        myToken.name = string(
            abi.encodePacked("lsSPLP-", token0name, "-", token1name)
        );
        myToken.symbol = string(
            abi.encodePacked("lsSPLP-", token0symbol, "-", token1symbol)
        );
    ...
    function _mint(address recipient, uint256 amount) internal {
        bytes[] memory _x;
        safeMintToken(lpToken, recipient, amount, _x);
        if (recipient != address(this)) {
            HederaTokenService.associateToken(recipient, lpToken);
            safeTransferTokenRouter(lpToken, address(this), recipient, amount);
        }
    }
    ...
    function mint(
        bytes calldata data
    ) public override nonReentrant returns (uint256 liquidity) {
    ...
    function _burn(address sender, uint256 amount) internal {
        safeBurnToken(lpToken, sender, amount, new int64[](0));
    }
        ...
        returns (IPool.TokenAmount[] memory withdrawnAmounts)
    {
        ...
        (uint256 balance0, uint256 balance1) = _balance();
        uint256 liquidity = HederaIERC20(lpToken).balanceOf(address(this));
    ...
    {
        balance0 = bento.toAmount(
            token0,
            bento.balanceOf(token0, address(this)),
            false
        );
        balance1 = bento.toAmount(
            token1,
    ...
    function _computeLiquidity(
        ...
        unchecked {
            ...
            liquidity = _computeLiquidityFromAdjustedBalances(
                adjustedReserve0,
                adjustedReserve1
            );
        }
    ...
    function _mintFee(
        ...
        _totalSupply = HederaIERC20(lpToken).totalSupply();
    ...
    function _getAmountOut(
        ...
        unchecked {
            uint256 adjustedReserve0 = (_reserve0 * 1e12) / decimals0;
            uint256 adjustedReserve1 = (_reserve1 * 1e12) / decimals1;
            uint256 feeDeductedAmountIn = amountIn -
                (amountIn * swapFee) /
                MAX_FEE;
            uint256 xy = _k(adjustedReserve0, adjustedReserve1);
            if (token0In) {
                uint256 x0 = adjustedReserve0 +
                    ((feeDeductedAmountIn * 1e12) / decimals0);
                uint256 y = _get_y(x0, xy, adjustedReserve1);
                dy = adjustedReserve1 - y;
                dy = (dy * decimals1) / 1e12;
            } else {
                uint256 x0 = adjustedReserve1 +
                    ((feeDeductedAmountIn * 1e12) / decimals1);
    ...
    function getAmountOut(
        ...
        if (tokenIn == token0) {
            finalAmountOut = bento.toShare(
                token1,
                _getAmountOut(amountIn, _reserve0, _reserve1, true),
                false
            );
        } else {
            if (tokenIn != token1) revert InvalidInputToken();
            finalAmountOut = bento.toShare(
                token0,
                _getAmountOut(amountIn, _reserve0, _reserve1, false),
                false
            );
        }
    }
    ...
    function getNativeReserves()
        public
        view
        returns (uint256 _nativeReserve0, uint256 _nativeReserve1)
    {
        return _getReserves();
    }
