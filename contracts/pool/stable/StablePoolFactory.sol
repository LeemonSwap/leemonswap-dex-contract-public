...
contract StablePoolFactory is IStablePoolFactory, PoolDeployer {
    bytes32 public constant bytecodeHash =
        keccak256(type(StablePool).creationCode);
    ...
    function deployPool(
        ...
        (address tokenA, address tokenB, uint256 swapFee) = abi.decode(
            _deployData,
        ...
        pool = address(new StablePool{salt: salt, value: msg.value}());
    ...
    function getDeployData()
        ...
        view
        override
        returns (bytes memory, IMasterDeployerV2)
    {
